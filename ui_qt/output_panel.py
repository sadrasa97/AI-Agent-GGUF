...
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QLabel, QToolButton, QMenu,
...
class TerminalOutputEdit(QPlainTextEdit):
    ...
    def keyPressEvent(self, event):
        ...
        if key in (Qt.Key_Return, Qt.Key_Enter):
            ...
            self._buffer = ""
            if cmd: self._submit_callback(cmd)
            return
        if key == Qt.Key_Backspace:
            if self._buffer: self._buffer = self._buffer[:-1]; super().keyPressEvent(event)
            return
        if text and text.isprintable(): self._buffer += text; super().keyPressEvent(event); return
        if key in {Qt.Key_Left, Qt.Key_Right, Qt.Key_Up, Qt.Key_Down, Qt.Key_Home, Qt.Key_End, Qt.Key_PageUp, Qt.Key_PageDown, Qt.Key_Delete}: return
...
class OutputPanel(QWidget):
    ...
    def __init__(self, workspace_path: Path, settings: Settings, parent=None):
        ...
        title.setStyleSheet("color: #5A647D; font-weight: 700; font-size: 10px; letter-spacing: 1.5px;")
        top.addWidget(title)
        top.addStretch()
        ...
        btn_style = """
            QToolButton { background: #161A26; color: #8B94A7; border: 1px solid #252B3D; border-radius: 6px; padding: 4px 10px; font-size: 11px; font-weight: 600; }
            QToolButton:hover { background: #252B3D; color: #E2E8F0; border-color: #3A4466; }
        ...
        self.shell_btn.setToolTip("Choose shell")
        ...
        self.output.setStyleSheet("QPlainTextEdit { background:#090A0F; color:#E2E8F0; border:none; padding:10px; }")
        ...
        self.input_line.setStyleSheet("QLineEdit { background: #161A26; color: #E2E8F0; border: 1px solid #252B3D; border-radius: 6px; padding: 6px 12px; font-size: 12px; } QLineEdit:focus { border: 1px solid #00E5FF; }")
        bottom.addWidget(self.input_line, stretch=1)
        self.run_btn = QPushButton("Run")
        self.run_btn.clicked.connect(self._send_input_command)
        self.run_btn.setStyleSheet("""
            QPushButton { background: #1E2333; color: #00E5FF; padding: 6px 16px; border: 1px solid #252B3D; border-radius: 6px; font-weight: 700; font-size: 11px; }
            QPushButton:hover { background: #252B3D; color: #FFFFFF; border-color: #3A4466; }
        """)
    ...
    def set_working_directory(self, path: Path):
        ...
        if self.process.state() == QProcess.Running: self._send_cd(self.workspace_path)

    def execute_command(self, command: str):
        if not command.strip(): return
        if command.strip().lower().startswith(AI_PREFIX):
            self._handle_ai_task(command); return
        if self.process.state() != QProcess.Running: self._restart_terminal(self._current_shell)
        if self.process.state() != QProcess.Running:
            self.output.appendPlainText("[terminal] shell is not running."); return
        self.process.write((command + "\n").encode("utf-8"))

    def _handle_ai_task(self, command: str):
        task_prompt = command.strip()[len(AI_PREFIX):].strip()
        if not task_prompt: self.output.appendPlainText("[ai] usage: /ai <task description>"); return
        if self.settings.backend == "gguf" and not self.settings.model_path:
            self.output.appendPlainText("[ai] GGUF model path is empty. Set model in Settings first."); return
        self.output.appendPlainText(f"[ai] task: {task_prompt}")
        self.output.appendPlainText("[ai] generating code...")
        wrapped_prompt = (
            "Write a complete Python solution for this task. Return exactly one runnable Python code block. "
            "Do not include any text outside the code block.\n\n" f"Task: {task_prompt}"
        )
        history = [{"role": "user", "content": wrapped_prompt}]
        workspace_context = build_workspace_context(self.workspace_path, user_text=task_prompt, max_files=80, max_bytes=6000, max_depth=4)
        try: provider = create_provider(self.settings)
        except ProviderError as exc: self.output.appendPlainText(f"[ai] error: {exc}"); return
        response = ""
        try:
            for tok in provider.stream(history, workspace_context=workspace_context): response += tok
        except ProviderError as exc:
            self.output.appendPlainText(f"[ai] provider error: {exc}"); provider.close(); return
        except Exception as exc:
            self.output.appendPlainText(f"[ai] unexpected error: {exc}"); provider.close(); return
        finally: provider.close()
        code = self._extract_python_code(response)
        if not code:
            self.output.appendPlainText("[ai] no python code block was generated.")
            if response.strip():
                self.output.appendPlainText("[ai] raw response:"); self.output.appendPlainText(response.strip())
            return
        path = self._next_task_file()
        try: path.parent.mkdir(parents=True, exist_ok=True); path.write_text(code, encoding="utf-8")
        except Exception as exc: self.output.appendPlainText(f"[ai] save error: {exc}"); return
    ...
    def _extract_python_code(text: str) -> str:
        ...
        for lang, code in blocks:
            if lang in {"python", "py"}: return code.strip()
        if blocks: return blocks[0][1].strip()
        return ""
    ...
    def execute_file(self, path: Path) -> bool:
        ...
        if ext == ".py": cmd = f'& "{py}" "{p}"' if shell == "PowerShell" else f'"{py}" "{p}"'
        elif ext == ".js": cmd = f'node "{p}"'
        elif ext == ".ts": cmd = f'npx ts-node "{p}"'
        elif ext == ".sh": cmd = f'bash "{p}"'
        elif ext == ".rb": cmd = f'ruby "{p}"'
        elif ext == ".php": cmd = f'php "{p}"'
        elif ext == ".go": cmd = f'go run "{p}"'
        elif ext == ".lua": cmd = f'lua "{p}"'
        elif ext == ".pl": cmd = f'perl "{p}"'
        elif ext == ".r": cmd = f'Rscript "{p}"'
        else: return False
    ...
    def _send_input_command(self):
        ...
        self.execute_command(command); self.input_line.clear()

    def _restart_terminal(self, shell_name: str):
        if self.process.state() != QProcess.NotRunning:
            self._intentional_restart = True; self.process.kill(); self.process.waitForFinished(1000)
        ...
        if not self.process.waitForStarted(3000):
            self.output.appendPlainText(f"[terminal] failed to start {shell_name}."); return

    def _send_cd(self, path: Path):
        shell_name = self._current_shell
        if shell_name == "Command Prompt": cmd = f'cd /d "{path}"'
        else:
    ...
    def _on_stdout(self):
        ...
        if data: self.output.appendPlainText(data.rstrip("\n"))
    ...
    def _on_finished(self, exit_code: int, _status):
        if self._intentional_restart: self._intentional_restart = False; return
        self.output.appendPlainText(f"[terminal exited with code {exit_code}]")

    def shutdown(self):
        if self.process.state() == QProcess.NotRunning: return
        self._intentional_restart = True; self.process.terminate()
        if not self.process.waitForFinished(800): self.process.kill(); self.process.waitForFinished(800)
